...
async function getDraft(userId: string): Promise<Draft | null> {
  const row = await runtime().DB.prepare("SELECT stage, payload FROM bot_drafts WHERE telegram_user_id = ?1")
    .bind(userId)
...
async function saveDraft(userId: string, draft: Draft | null) {
  ...
  await runtime().DB.prepare(
    `INSERT INTO bot_drafts (telegram_user_id, stage, payload, updated_at)
     VALUES (?1, ?2, ?3, ?4) ON CONFLICT(telegram_user_id) DO UPDATE SET stage=?2, payload=?3, updated_at=?4`
  )
    .bind(userId, draft.stage, JSON.stringify(draft.payload), new Date().toISOString())
    .run();
}
...
async function handleAdmin(message: TelegramMessage, user: TelegramUser, text: string) {
  ...
  if (draft.stage in next) {
    ...
    if (
      (numeric && (!Number.isInteger(value) || Number(value) < (draft.stage === "age" ? 18 : 0))) ||
      (!numeric && !value)
    ) {
  ...
  if (draft.stage === "bio") {
    ...
    const profile = await createProfile({
      ...
      age: Number(draft.payload.age),
      zone: String(draft.payload.zone),
      price: Number(draft.payload.price),
      commission: Number(draft.payload.commission),
      bio: String(draft.payload.bio),
      status: "draft",
    });
    await saveDraft(userId, null);
    return say(
      message.chat.id,
      `✅ Perfil <b>${profile.name}</b> creado como borrador.\n\nUsa /panel para revisarlo y publicarlo.`
    );
  }
}
...
async function handleCustomer(message: TelegramMessage, user: TelegramUser, text: string) {
  ...
  if (!match) {
    return say(
      message.chat.id,
      "Bienvenido al catálogo. Elige un perfil en la página y pulsa “Solicitar disponibilidad”."
    );
  }
  const profile = await getProfile(match[1]);
  if (!profile || !["available", "busy"].includes(profile.status)) {
    return say(message.chat.id, "Este perfil ya no está publicado.");
  }
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (adminIds().has(String(user.id))) {
    await handleAdmin(message, user, text);
  } else {
    await handleCustomer(message, user, text);
  }
